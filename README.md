...
- `Shift` のダブルタップで、どのSpaceからでも入力パレットを表示・非表示
- ダブルタップの2回目を1秒長押しするとローカル音声入力を開始。認識辞書と置換ルールにも対応
...
- 回答の生成中も次の質問を入力可能。Enterで確定した質問はFIFOキューで順番に実行
- 入力欄は文章の折り返しに合わせて最大7行まで伸縮。Shift+Enterで明示的に改行
- 最後の利用から60秒後にモデルをメモリから解放
- 質問履歴、システムプロンプト、モデル選択をローカル保存
- 対応するMacではApple Foundation Modelsも選択可能
- Grok BuildのサブスクリプションでGrok 4.5、Web / X検索、Mac内の読み取り専用ファイル検索を選択可能
...
Grokはユーザーのホームディレクトリを起点に、macOSの読み取り専用sandbox内でヘッドレス実行されます。Mac内のファイル検索・読み取りとGrok側のホスト型Web / X検索を利用できます。ファイル編集・作成・削除、シェル、MCPは拒否します。読み取ったファイル内容は、回答生成のためxAIへ送信される場合があります。

## 使い方

- `Shift`を素早く2回: 入力パレットを表示・非表示
- `Shift`のダブルタップで2回目を1秒長押し: パレットを表示して音声入力を開始
- マイクボタン: ローカル音声入力の開始 / 停止（macOS 26以降）
- `Enter`: 質問を送信
- `Esc`またはパレット外をクリック: 閉じる
- メニューバーのLocalLLMアイコン: 質問、履歴、設定、終了

設定の「音声入力」タブでは、Apple SpeechAnalyzerへ渡す認識辞書を1行1語で登録できます（最大100件）。表記を直したい場合は、置換ルールを `グロック => Grok` の形式で1行ずつ登録します。置換は上から順に、途中表示と確定結果の両方へ適用されます。
